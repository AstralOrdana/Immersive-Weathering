...
public class WaxedExposedRustableDoorBlock extends DoorBlock {
    protected WaxedExposedRustableDoorBlock(Settings settings) {
        super(settings);
    }
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
        ...
        if (hasPower != state.get(POWERED)) { // checks if redstone input has changed
            if (hasPower) { // if the door is now being powered, open right away
                ...
                world.createAndScheduleBlockTick(pos, this, 10); // 0.5 second
            }
