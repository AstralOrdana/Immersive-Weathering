...
import com.ordana.immersive_weathering.registry.blocks.ForestLichenBlock;
...
public class ForestLichenFeature extends Feature<GlowLichenFeatureConfig> {
    public ForestLichenFeature(Codec<GlowLichenFeatureConfig> codec) {
        super(codec);
    }
            ...
            if (generate(structureWorldAccess, blockPos, structureWorldAccess.getBlockState(blockPos), glowLichenFeatureConfig, random, list)) {
                ...
                for (Direction direction : list) {
                    ...
                    for (int i = 0; i < glowLichenFeatureConfig.searchRange; ++i) {
                        ...
                        if (isAirOrWater(blockState) && !blockState.isOf(ModBlocks.FOREST_LICHEN)) {
                            break;
                        }
    ...
    public static boolean generate(StructureWorldAccess world, BlockPos pos, BlockState state, GlowLichenFeatureConfig config, Random random, List<Direction> directions) {
        ...
        ForestLichenBlock forestLichenBlock = (ForestLichenBlock) ModBlocks.FOREST_LICHEN;
        BlockState blockState2 = forestLichenBlock.withDirection(state, world, pos, direction);
        if (blockState2 == null) {
            return false;
        } else {
            ...
            if (random.nextFloat() < config.spreadChance) {
                forestLichenBlock.trySpreadRandomly(blockState2, world, pos, direction, random, true);
            }
