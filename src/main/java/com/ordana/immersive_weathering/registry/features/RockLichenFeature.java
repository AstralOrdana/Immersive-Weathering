...
import com.ordana.immersive_weathering.registry.blocks.RockLichenBlock;
...
public class RockLichenFeature extends Feature<GlowLichenFeatureConfig> {
    public RockLichenFeature(Codec<GlowLichenFeatureConfig> codec) {
        super(codec);
    }
            ...
            if (generate(structureWorldAccess, blockPos, structureWorldAccess.getBlockState(blockPos), glowLichenFeatureConfig, random, list)) {
                ...
                for (Direction direction : list) {
                    ...
                    for (int i = 0; i < glowLichenFeatureConfig.searchRange; ++i) {
                        ...
                        if (isAirOrWater(blockState) && !blockState.isOf(ModBlocks.ROCK_LICHEN)) {
                            break;
                        }
    ...
    public static boolean generate(StructureWorldAccess world, BlockPos pos, BlockState state, GlowLichenFeatureConfig config, Random random, List<Direction> directions) {
        ...
        RockLichenBlock rockLichenBlock = (RockLichenBlock) ModBlocks.ROCK_LICHEN;
        BlockState blockState2 = rockLichenBlock.withDirection(state, world, pos, direction);
        if (blockState2 == null) {
            return false;
        } else {
            ...
            if (random.nextFloat() < config.spreadChance) {
                rockLichenBlock.trySpreadRandomly(blockState2, world, pos, direction, random, true);
            }
