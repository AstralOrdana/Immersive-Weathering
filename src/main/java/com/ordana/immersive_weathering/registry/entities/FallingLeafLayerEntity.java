...
public class FallingLeafLayerEntity extends ImprovedFallingBlockEntity {

    public FallingLeafLayerEntity(EntityType<FallingLeafLayerEntity> type, World level) {
        super(type, level);
    }

    public FallingLeafLayerEntity(World level) {
        super(ModEntities.FALLING_LEAF_LAYER, level);
    }

    public FallingLeafLayerEntity(World level, BlockPos pos, BlockState blockState) {
        super(ModEntities.FALLING_LEAF_LAYER, level, pos, blockState, false);
    }

    public static FallingLeafLayerEntity fall(World level, BlockPos pos, BlockState state) {
        FallingLeafLayerEntity entity = new FallingLeafLayerEntity(level, pos, state);
                    ...
                    if (!onState.isOf(Blocks.MOVING_PISTON)) {
                        ...
                        if (canBeReplaced && canSurvive) {
                            ...
                            if (onState.isOf(blockState.getBlock())) {
                                int layers = blockState.get(LeafPileBlock.LAYERS);
                                int toLayers = onState.get(LeafPileBlock.LAYERS);
                                int total = layers + toLayers;
                                int target = MathHelper.clamp(total, 1, 8);
                                remaining = total - target;
                                blockState = blockState.with(LeafPileBlock.LAYERS, target);
                            }
                            ...
                            if (this.world.setBlockState(pos, blockState, 3)) {
                                ...
                                if (remaining != 0) {
                                    ...
                                    blockState = blockState.with(LeafPileBlock.LAYERS, remaining);
    ...
    public static boolean isFree(BlockState pState) {
        ...
        return pState.isAir() || pState.isIn(BlockTags.FIRE) || material.isLiquid() || (material.isReplaceable() && !(pState.getBlock() instanceof LeafPileBlock));
    }
