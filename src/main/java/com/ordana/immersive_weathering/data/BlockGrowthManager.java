...
public class BlockGrowthManager extends SimpleJsonResourceReloadListener {
    ...
    private static final Map<Block, BlockGrowthConfiguration> GROWTH_FOR_BLOCK = new HashMap<>();

    public BlockGrowthManager() {
        super(GSON, "block_growths");
    }

    public static Optional<BlockGrowthConfiguration> getBlockGrowthConfig(Block block) {
        return Optional.ofNullable(GROWTH_FOR_BLOCK.get(block));
    }

    public static void performGrowth(BlockState state, ServerLevel level, BlockPos pos) {
        getBlockGrowthConfig(state.getBlock()).ifPresent(c ->{
          if(c.biomes().isEmpty() || c.biomes().get().contains(level.getBiome(pos))){
              c.grow(pos, level);
          }
        });
    }
    ...
    protected void apply(Map<ResourceLocation, JsonElement> jsons, ResourceManager manager, ProfilerFiller profile) {
        ...
        for (var e : jsons.entrySet()) {
            ...
            o.ifPresent(config -> GROWTH_FOR_BLOCK.put(config.getOwner(), config));
        }
    }
}
