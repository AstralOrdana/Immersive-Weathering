...
public class FrostFeature extends Feature<GlowLichenConfiguration> {
    public FrostFeature(Codec<GlowLichenConfiguration> codec) {
        super(codec);
    }
            ...
            if (generate(structureWorldAccess, blockPos, structureWorldAccess.getBlockState(blockPos), glowLichenFeatureConfig, random, list)) {
                ...
                for (Direction direction : list) {
                    ...
                    for (int i = 0; i < glowLichenFeatureConfig.searchRange; ++i) {
                        ...
                        if (isAirOrWater(blockState) && !blockState.is(ModBlocks.FROST.get())) {
                            break;
                        }
    ...
    public static boolean generate(WorldGenLevel world, BlockPos pos, BlockState state, GlowLichenConfiguration config, RandomSource random, List<Direction> directions) {
        ...
        FrostBlock frostBlock = (FrostBlock) ModBlocks.FROST.get();
        BlockState blockState2 = frostBlock.getStateForPlacement(state, world, pos, direction);
        if (blockState2 == null) {
            return false;
        } else {
            ...
            if (random.nextFloat() < config.chanceOfSpreading) {
                frostBlock.spreadFromFaceTowardRandomDirection(blockState2, world, pos, direction, random, true);
            }
