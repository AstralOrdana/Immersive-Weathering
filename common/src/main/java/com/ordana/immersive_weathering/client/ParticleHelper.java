...
public class ParticleHelper {
    ...
    public static void spawnLeavesParticles(BlockState state, Level level, BlockPos pos, Random random) {
        ...
        if(state.is(ModTags.VANILLA_LEAVES) && !state.getValue(LeavesBlock.PERSISTENT)) {
            if (random.nextInt(32) == 0 && !level.getBlockState(pos.below()).isCollisionShapeFullBlock(level, pos)) {
                ParticleUtils.spawnParticlesOnBlockFaces(level, pos, leafParticle, UniformInt.of(0, 1));
            }
        }
    }
}
